...
Nesta seção, analisamos o esforço computacional, medido através do número de comparações, das operações de adição e remoção para as estruturas Árvore AVL, Árvore Rubro-Negra e Árvore B (nas ordens 1, 5 e 10).

\subsection{Análise da Operação de Adição}

Conforme ilustrado na Figura 1, a Árvore Rubro-Negra apresentou o desempenho mais eficiente. Este resultado decorre do seu critério de balanceamento menos rígido em comparação à AVL. Enquanto a AVL realiza rotações frequentes para garantir que a diferença de altura entre subárvores seja mínima, a Rubro-Negra aceita um desbalanceamento maior, o que reduz o custo de manutenção da estrutura.

Sobre a Árvore B, observa-se uma relação inversa entre a ordem e o número de comparações: ordens menores exigiram menos comparações do que ordens maiores. Embora ordens maiores reduzam a altura da árvore, o custo de realizar a busca binária dentro de um nó com um vetor de chaves extenso supera a economia obtida pela redução do número de nós visitados.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{insercao.jpeg}
    \caption{Comparação do número de comparações na operação de adição.}
\end{figure}

\subsection{Análise da Operação de Remoção}
Conforme ilustrado na figura 2, a operação de remoção, sendo a mais complexa, evidenciou o custo estrutural de cada árvore. A Árvore B de ordem 1 apresentou o maior custo de processamento. Como o nó possui uma capacidade reduzida, a condição de \textit{underflow} é atingida com frequência, forçando o algoritmo a realizar diversas operações de fusão e empréstimo de chaves. A Árvore Rubro-Negra, novamente, mostrou-se a estrutura mais equilibrada para cenários de alta rotatividade, mantendo um custo previsível e otimizado.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{remocao.jpeg}
    \caption{Comparação do número de comparações na operação de remoção.}
\end{figure}

\newpage
\section{Considerações Finais}
O estudo demonstrou que a eficiência de uma estrutura de dados não depende apenas da sua complexidade assintótica teórica ($O(\log n)$), mas da sua implementação prática. Enquanto a AVL é eficiente para buscas, a Rubro-Negra demonstrou superioridade prática em inserções e remoções devido à redução de custos de balanceamento. 

Para a Árvore B, conclui-se que o parâmetro de ordem atua como um ajuste fino (\textit{fine-tuning}): ordens muito pequenas elevam o custo de manutenção estrutural (\textit{underflow}), enquanto ordens muito grandes elevam o custo de processamento local (busca binária em vetores longos). O desempenho ideal, portanto, depende do perfil de acesso da aplicação, sendo que estruturas com rebalanceamento flexível tendem a ser mais robustas em cenários dinâmicos.
